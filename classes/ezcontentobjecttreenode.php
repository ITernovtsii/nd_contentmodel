...
class eZContentObjectTreeNode extends eZPersistentObject
{
    ...
    /*!
     Constructor
    */
    public function eZContentObjectTreeNode($row = array())
    {
        ...
        static $definition = array(
            "fields"              => array(
                ...
                'contentobject_version'      => array(
                    'name'     => 'ContentObjectVersion',
                    'datatype' => 'integer',
                    'default'  => 0,
                    'required' => true
                ),
                'contentobject_is_published' => array(
                    'name'     => 'ContentObjectIsPublished',
                    'datatype' => 'integer',
                    'default'  => 0,
                    'required' => true
                ),
                ...
                'modified_subnode'           => array(
                    'name'     => 'ModifiedSubNode',
                    'datatype' => 'integer',
                    'default'  => 0,
                    'required' => true
                ),
                ...
                "path_identification_string" => array(
                    'name'     => "PathIdentificationString",
                    'datatype' => 'text',
                    'default'  => '',
                    'required' => true
                ),
    ...
    */
    static function createPermissionCheckingSQL($limitationList, $treeTableName = 'ezcontentobject_tree', $tableAliasName = 'ezcontentobject_tree')
    {
        ...
        $sqlPermissionChecking = array(
            'from'        => $sqlPermissionCheckingFrom,
    ...
    */
    static function getLimitationList(&$limitation)
    {
        ...
        if ($limitation !== false) {
            ...
            if (isset($GLOBALS['ezpolicylimitation_list'][$currentUserID]['content']['read'])) {
    ...
    */
    static function subTreeByNodeID($params = false, $nodeID = 0)
    {
        ...
        if (!$offset && !$limit) {
            ...
            $nodeListArray = $db->arrayQuery($query, array(
                'offset' => $offset,
                'limit'  => $limit
            ),
                $server);
    ...
    */
    static function sortArrayBySortFieldAndSortOrder($sortField, $sortOrder)
    {
        return array(
            array(
                eZContentObjectTreeNode::sortFieldName($sortField),
                $sortOrder
            )
        );
    }
    ...
    */
    static function assignSectionToSubTree($nodeID, $sectionID, $oldSectionID = false)
    {
        ...
        $object  = $node->object();
        eZAudit::writeAudit('section-assign', array(
            'Section ID'          => $sectionID, 'Section name' => $section->attribute('name'),
            'Node ID'             => $nodeID,
    ...
    */
    ...
    /**
     * @param int        $objectID
     * @param bool|false $asObject
     * @return eZContentObjectTreeNode|null
     */
    public static function findMainNode($objectID, $asObject = false)
    {
        $objectID      = (int)$objectID;
        $query         = "SELECT ezcontentobject.*,
                           ezcontentobject_tree.*,
     ...
     * @param int|bool|false $nodeID
     * @param string|bool|false $lang
     * @param bool|true  $asObject
     * @param array|bool|false $conditions
     * @return eZContentObjectTreeNode|null|eZContentObjectTreeNode[]|array
     */
    public static function fetch($nodeID = false, $lang = false, $asObject = true, $conditions = false)
    {
        $returnValue = null;
        $db          = eZDB::instance();
    ...
    */
     ...
     * @return null|eZContentObjectTreeNode An eZContentObjectTreeNode object or null if no node was found.
     */
    public static function fetchNode($contentObjectID, $parentNodeID)
    {
        $returnValue     = null;
        $db              = eZDB::instance();
    ...
    {
        return static::fetch($this->attribute('parent_node_id'));
    }
    ...
    /**
     * @param                $nodePath
     * @param bool|false     $withLastNode If true - last node in the path is included in the list. The last node is the node which the path was fetched from.
     * @param bool|true      $asObjects    If true then return PHP objects, if not return raw row data.
     * @param int|bool|false $limit        Maximum number of nodes in the path to use, starting from last node
     * @return array|eZContentObjectTreeNode[] An array with content node objects that is present in the node path $nodePath.
     */
    public static function fetchNodesByPathString($nodePath, $withLastNode = false, $asObjects = true, $limit = false)
    {
    ...
    /*!
     ...
     \endcode
    */
    public static function fetchClassIdentifierListByPathString($nodePath, $withLastNode, $limit = false)
    {
        $itemList = array();
     ...
     * @return mixed
     */
    public function createObject($contentClassID, $parentNodeID = 2)
    {
    ...
    */
    function removeThis()
    {
        ...
        if (eZAudit::isAuditEnabled()) {
            ...
            eZAudit::writeAudit('content-delete', array(
                'Node ID' => $nodeIDAudit, 'Object ID' => $objectID, 'Content Name' => $objectName,
    ...
    */
    static function removeSubtrees($deleteIDArray, $moveToTrash = true, $infoOnly = false)
    {
        ...
        foreach ($deleteIDArray as $deleteID) {
            ...
            $item           = array(
                "nodeName"           => $nodeName, // Backwards compatibility
        ...
        return array(
            'move_to_trash'         => $moveToTrashAllowed,
    ...
    */
    function move($newParentNodeID, $nodeID = 0)
    {
        ...
        if ($oldParentNodeID != $newParentNodeID) {
            ...
            $object = $node->object();
            eZAudit::writeAudit('content-move', array(
                'Node ID'            => $node->attribute('node_id'),
        ...
        foreach ($array as $node) {
            ...
            if ($with_contentobject) {
                if (isset($node['class_name'])) {
                    ...
                    $className     = eZContentObject::getClassName(isset($node['class_identifier']) ? $node['class_identifier'] : null);
                    $contentObject = new $className($node);
                    ...
                    $className     = eZContentObject::getClassName(isset($node['class_identifier']) ? $node['class_identifier'] : null);
                    $contentObject = new $className(array());
                    if (isset($node['name'])) {
    ...
    */
    static function unserialize($contentNodeDOMNode, $contentObject, $version, $isMain, &$nodeList, &$options, $handlerType = 'ezcontentobject')
    {
        ...
        if (is_object($node)) {
            ...
            switch ($choosenAction) {
                ...
                default: {
                    ...
                    $options['error'] = array(
                        'error_code'  => eZContentObject::PACKAGE_ERROR_NODE_EXISTS,
                        'element_id'  => $remoteID,
        ...
        if ($parentNodeID == -1 && $parentNodeRemoteID) {
            ...
            $options['suspended-nodes'][$parentNodeRemoteID] = array(
                'nodeinfo' => $nodeInfo,
    ...
    */
    ...
    public function attribute($attr, $noFunction = false)
    {
        if (!$noFunction) {
            ...
            if (!isset($def['fields'][$attr])) {
                $object = $this->object();
                if ($object instanceof eZContentObject && $object->hasCallbackAttribute($attr)) {
                    return $object->{$attr};
                }
            }
        }
    ...
    {
        $has = parent::hasAttribute($attr);
        if (!$has) {
            $object = $this->object();
            $has    = $object instanceof eZContentObject && $object->hasCallbackAttribute($attr);
        }
    ...
    public function __isset($name)
    {
        ...
        return $object instanceof eZContentObject && isset($object->{$name});
    }
    ...
    {
        $object = $this->object();
        if ($object instanceof eZContentObject && $object->hasCallbackAttribute($name)) {
            return $object->{$name};
        }
    ...
    {
        $object = $this->object();
        if ($object instanceof eZContentObject && $object->hasCallbackMethod($method)) {
            return call_user_func_array(array($object, $method), $args);
        }

        throw new Exception('Call to undefined method eZContentObjectTreeNode::' . $method);
    }
