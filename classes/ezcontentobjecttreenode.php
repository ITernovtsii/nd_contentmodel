...
//        $object->setAttribute( "main_node_id", $node->attribute( 'node_id' ) );
    ...
    */
    ...
    public function attribute($attr, $noFunction = false)
    {
        if (!$noFunction) {
            $object = $this->object();
            if ($object instanceof eZContentObject && $object->hasCallAttribute($attr)) {
                return $object->{$attr};
            }
        }
    ...
    public function hasAttribute($attr)
    {
        ...
        if (!$has) {
            $object = $this->object();
            $has    = $object instanceof eZContentObject && $object->hasCallAttribute($attr);
        }
    ...
    {
        $object = $this->object();
        if ($object instanceof eZContentObject && $object->hasCallAttribute($name)) {
            return $object->{$name};
        }
