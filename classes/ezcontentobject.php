...
/**
 * Override of default class eZContentObject
 * Used for ContentModels
 * @class     eZContentObject
 * @author    Ivan Ternovtsiy <it@norse.digital>
 * @date      06 Nov 2015
 * @copyright Norse Digital SA 2015
 */
class eZContentObject extends eZPersistentObject
{
    /**
     * @var array Ex. array('name' => 'getCustomName')
     */
    ...
    public function __construct($row)
    {
        ...
        if (isset($row['content_translation'])) {
            ...
            } elseif (isset($row['language_mask'])) {
     ...
     * @return string
     */
    public static function getClassName($identifier)
    {
        if (is_int($identifier)) {
            $identifier = eZContentClass::classIdentifierByID($identifier);
        }
        $className = 'eZContentObject';
        if (!empty($identifier)) {
            $modelClass = 'eZContentModel' . str_replace(' ', '', ucwords(str_replace('_', ' ', $identifier)));
    ...
    */
    public function matchIngroupIDList()
    {
    ...
    */
    static function create($name, $contentclassID, $userID, $sectionID = 1, $version = 1, $languageCode = false)
    {
        ...
        $className = self::getClassName($contentclassID);
    ...
    */
    function purge()
    {
        ...
        eZAudit::writeAudit('content-delete', array(
            'Object ID' => $delID, 'Content Name' => $this->attribute('name'),
...
//             print( "Cached<br>" );
    ...
    */
    ...
    {
        $this->InputRelationList = array(
            eZContentObject::RELATION_EMBED => array(),
        ...
        $inputParameters = false, $parameters = array())
    {
        ...
        if (!$inputParameters) {
            $inputParameters = array(
                'unvalidated-attributes' => &$unvalidatedAttributes,
        ...
        foreach ($contentObjectAttributes as $contentObjectAttribute) {
            ...
            $statusMap[$contentObjectAttribute->attribute('id')] = array(
                'value'     => $status,
    ...
    */
        ...
        $reverseRelatedObjects = false)
    {
        ...
        if (!$offset && !$limit) {
            ...
            $relatedObjects = $db->arrayQuery($query, array(
                'offset' => $offset,
                'limit'  => $limit
            ));
        }
    ...
    */
    function createNodeAssignment($parentNodeID, $isMain, $remoteID = false, $sortField = eZContentObjectTreeNode::SORT_FIELD_PUBLISHED, $sortOrder = eZContentObjectTreeNode::SORT_ORDER_DESC)
    {
        $nodeAssignment = eZNodeAssignment::create(array(
            'contentobject_id'      => $this->attribute('id'),
            'contentobject_version' => $this->attribute('current_version'),
            'parent_node'           => $parentNodeID,
            'is_main'               => ($isMain ? 1 : 0),
    ...
    */
    static function unserialize($package, $domNode, &$options, $ownerID = false, $handlerType = 'ezcontentobject')
    {
        ...
        if (!$contentClass) {
            $options['error'] = array(
                'error_code'  => self::PACKAGE_ERROR_NO_CLASS,
                'element_id'  => $remoteID,
        ...
        if (!$contentObject) {
            ...
            switch ($choosenAction) {
                ...
                default: {
                    $options['error'] = array(
                        'error_code'  => self::PACKAGE_ERROR_EXISTS,
                        'element_id'  => $remoteID,
                        'description' => $description,
                        'actions'     => array(
                            self::PACKAGE_REPLACE => ezpI18n::tr('kernel/classes', 'Replace existing object'),
        ...
        foreach ($versionListNode->getElementsByTagName('version') as $versionDOMNode) {
            ...
            $versionList[$versionDOMNode->getAttributeNS('http://ez.no/ezobject', 'version')] = array(
                'node_list' => $nodeList,
    ...
    */
    ...
    {
        if (!$noFunction && isset($this->callbackAttributes[$attr])) {
            return $this->{$attr};
        }
    ...
    {
        return isset($this->callbackAttributes[$name]) && $this->{$name} !== null;
    }
